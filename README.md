...
- 无硬性大小限制（已取消单文件上限），实际可用大小受浏览器与宿主内存约束，建议单文件不超过约 500MB；超大文件建议直接粘贴路径由 AI 用文件系统读取。
...
**Q: 大文件支持吗？**
A: 支持，已取消单文件大小限制（不再有 37MB 上限）。实际可用大小受浏览器与宿主内存约束：文件经 base64 传输会膨胀约 33%，浏览器 `FileReader` 与 DSH 进程都需在内存中驻留整份文件，建议单文件不超过约 500MB。超大文件（如数百 MB 以上的 TIFF/扫描 PDF）建议直接将路径粘贴到输入框，由 AI 用文件系统读取，完全不受上传机制限制。
